...
### film_director
Содержит информацию о том, какие режисёры из таблицы `directors` относятся к фильму.
Таблица включает поля:
- составной первичный и внешний ключ `film_id` (отсылает к таблице `films`) - идентификатор фильма;
- составной первичный и внешний ключ `director_id` (отсылает к таблице `genres`) - идентификатор жанра;
### directors
Содержит инфрормацию о режисёрах фильмов
- первичный ключ `id` - идентификатор режисёра ;
- `name` - имя режисёра;